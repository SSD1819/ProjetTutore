...
Notre problématique étant de voir s'il existe un lien entre la façon d'enseigner et les réponses au test, nous avons voulu essayer de prédire la méthode d'enseignement à l'aide des réponses des élèves avec la régression logistique. Cette méthode permet de modéliser une classification, à l'aide notamment de l'odds ratio <!-- partie à verifier --> 
...
La réalisation d'une ACM comme première approche sur le jeu de données permis de comprendre mieux ce qui différencie les individus dans notre jeu de données et à la fois d'avoir un premier résultats sur la différence entre les deux pédagogies selon cette méthode.
...
```{r include=TRUE}
plot.HCPC(res.hcpcmca,choice = "bar",title = "Diagramme des gains d'inertie")
```
...
```{r include=TRUE}
corrplot(m.cor,method = "circle",title = "Matrice des corrélations")
```
...
```{r include=TRUE}
plot.PCA(res.pca.nonindsup,choix = "var",select = "contrib 6",title = "Cercle des corrélations du plan principal")
```
